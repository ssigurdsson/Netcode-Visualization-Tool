...
import sys
import socket
from core_game import Network, UserInputs
from config import *
import threading
import pickle
import time
...
class Server(Network):
	...
	def needs_sync(self):
		self.server_time = time.time()
		time_passed = self.server_time - self.last_transmit_time
		return time_passed > SERVER_TRANSMIT_INTERVAL
	...
	def _transmit_players(self, players, player_deaths):
		...
		for player_id, player in players.items():
			transmit.append((player_id, self.encode_player(player)))
		...
		for player_id, player in players.items():
			...
			if self.server_time - player_time >= TIMEOUT_LIMIT:
				...
				if player_id in player_deaths:
					...
					message = (transmit, player_ping, self.server_time)
		...
		while self.run:
			while self.ack_reception_queue:
				...
				if packet_id in unacked_packets:
					if unacked_packets[packet_id][0] == player_addr:
						del unacked_packets[packet_id]

			for packet_id, (player_addr, code, updates, past_server_time) in \
					list(unacked_packets.items()):
			...
			while self.ack_transmission_queue:
				player_addr, packet_id, server_time, code, updates = \
						self.ack_transmission_queue.popleft()
				unacked_packets[packet_id] = \
						(player_addr, code, updates, server_time)
	...
	def _add_new_player(self, player, player_addr):
		...
		if player_addr not in self.connected_addresses:
			...
			player_update = (self.player_id, self.decode_player(player))
			self.player_add_queue.append(player_update)
	...
	def _update_commands(self, player_inputs, player_addr):
		if player_addr in self.addr_to_id:
			player_id = self.addr_to_id[player_addr]
			update = (player_id, self.decode_inputs(player_inputs))
